...
#define ZM_LL_TRACE    0
#define ZM_LL_DEBUG    1
#define ZM_LL_INFO     2
#define ZM_LL_WARN     3
#define ZM_LL_ERROR    4
#define ZM_LL_FATAL    5


// ------------------------------ CONFIGURATION ------------------------------

//#define CPPLOG_FILTER_LEVEL               ZM_LL_WARN
//#define CPPLOG_SYSTEM_IDS
//#define CPPLOG_USE_SYSCALL_FOR_THREAD_ID
//#define CPPLOG_THREADING
#define CPPLOG_HELPER_MACROS
...
#ifndef ZM_CPPLOG_FILTER_LEVEL
#define ZM_CPPLOG_FILTER_LEVEL ZM_LL_DEBUG
#endif
...
namespace cpplog
{
	...
	public:
		//My_ostream() {}
		My_ostream(helpers::fixed_streambuf* bf): std::ostream(bf){}
	};

	template<typename _T>
	inline My_ostream& operator<<(My_ostream& ss, const _T& t) 
	{
		std::ostream* p_o = &ss;
		(*p_o) << t;
		return ss;
    ...
    private:
        void Init(const char* file, unsigned int line, loglevel_t logLevel, bool useDefaultLogFormat=true)
        {
			...
			cpplog::helpers::slocaltime(&gmt, &m_logData->messageTime);
        ...
        void Flush()
        {
            if( !m_flushed )
            {
                ...
                if( savedLogLevel == ZM_LL_FATAL && !getSetFatal(true, true) )
                {
                    // Set our fatal flag.
                    getSetFatal(false, true);

                    std::cout<<"++++   LL_TATAL exist(1)  ++++++"<<std::endl;
					throw 0;
                   // std::exit(1);
                    std::cout<<"after std::exist(1) LL_FAATAL throw"<<std::endl;
                    throw 0;

#ifdef _DEBUG
// Only exit in debug mode if CPPLOG_FATAL_EXIT_DEBUG is set.
#if defined(CPPLOG_FATAL_EXIT_DEBUG) || defined(CPPLOG_FATAL_EXIT)
                    //std::cout<<"+++ std::exit(1)"<<std::endl;
                  //  
                    std::cout<<"after std::exist(1) LL_FAATAL throw"<<std::endl;
                    throw 0;
					std::exit(1);
    ...
    */
    ...
    private:
        ...
        {
            // Create dummy item.
            m_dummyItem = new LogData(ZM_LL_TRACE);
    ...
    namespace templated
    {
        ...
        template <loglevel_t lowestLevel = ZM_LL_TRACE>
...
/*
#ifdef ZM_RELEASE
#define LOG_LEVEL(level, logger)    cpplog::LogMessage("AM", __LINE__, (level), logger).getStream()
#else
...
/*
#ifdef ZM_RELEASE
#define ZM_LOG_LEVEL(level, logger)    cpplog::LogMessage("AM", __LINE__, (level), logger).getStream()
#else
#define ZM_LOG_LEVEL(level, logger)    cpplog::LogMessage(__FILE__, __LINE__, (level), logger).getStream()
#endif
*/

//#define ZM_LOG_LEVEL(level, logger)    cpplog::LogMessage(__FILE__, __LINE__, (level), logger).getStream()

#define ZM_LOG_NOTHING(level, logger)  true ? (void)0 : cpplog::helpers::VoidStreamClass() & ZM_LOG_LEVEL(level, logger)

// Series of debug macros, depending on what we log.
#if ZM_CPPLOG_FILTER_LEVEL <= ZM_LL_TRACE
#define ZM_LOG_TRACE(logger)   ZM_LOG_LEVEL(ZM_LL_TRACE, logger)
#else
#define ZM_LOG_TRACE(logger)   ZM_LOG_NOTHING(ZM_LL_TRACE, logger)
#endif

#if ZM_CPPLOG_FILTER_LEVEL <= ZM_LL_DEBUG
#define ZM_LOG_DEBUG(logger)   ZM_LOG_LEVEL(ZM_LL_DEBUG, logger)
#else
#define ZM_LOG_DEBUG(logger)   ZM_LOG_NOTHING(ZM_LL_DEBUG, logger)
#endif

#if ZM_CPPLOG_FILTER_LEVEL <= ZM_LL_INFO
#define ZM_LOG_INFO(logger)    ZM_LOG_LEVEL(ZM_LL_INFO, logger)
#else
#define ZM_LOG_INFO(logger)    ZM_LOG_NOTHING(ZM_LL_INFO, logger)
#endif

#if ZM_CPPLOG_FILTER_LEVEL <= ZM_LL_WARN
#define ZM_LOG_WARN(logger)    ZM_LOG_LEVEL(ZM_LL_WARN, logger)
#else
#define ZM_LOG_WARN(logger)    ZM_LOG_NOTHING(ZM_LL_WARN, logger)
#endif

#if ZM_CPPLOG_FILTER_LEVEL <= ZM_LL_ERROR
#define ZM_LOG_ERROR(logger)   ZM_LOG_LEVEL(ZM_LL_ERROR, logger)
#else
#define ZM_LOG_ERROR(logger)   ZM_LOG_NOTHING(ZM_LL_ERROR, logger)
#endif

#define ZM_LOG_FATAL(logger) ZM_LOG_LEVEL(ZM_LL_FATAL, logger)


#ifdef _DEBUG
#define ZM_DLOG_INFO(logger) ZM_LOG_INFO(logger)
#define ZM_DLOG_FATAL(logger) ZM_LOG_FATAL(logger)
#else
#define ZM_DLOG_INFO(logger) ZM_LOG_NOTHING(ZM_LL_INFO, logger) 
#define ZM_DLOG_FATAL(logger) ZM_LOG_NOTHING(ZM_LL_FATAL, logger) 
#endif



//#define LOG_ZM_LL_INFO(logger)     ZM_LOG_INFO(logger)
//#define LOG_ZM_LL_WARN(logger)     ZM_LOG_WARN(logger)
//#define LOG_ZM_LL_ERROR(logger)    ZM_LOG_ERROR(logger)
//#define LOG_ZM_LL_FATAL(logger)    ZM_LOG_FATAL(logger)


#define ZM_LOG_ZM_LL_INFO(logger)     ZM_LOG_INFO(logger)
#define ZM_LOG_ZM_LL_WARN(logger)     ZM_LOG_WARN(logger)
#define ZM_LOG_ZM_LL_ERROR(logger)    ZM_LOG_ERROR(logger)
#define ZM_LOG_ZM_LL_FATAL(logger)    ZM_LOG_FATAL(logger)

#define ZM_DLOG_ZM_LL_INFO(logger)     ZM_DLOG_INFO(logger)
#define ZM_DLOG_ZM_LL_WARN(logger)     ZM_DLOG_WARN(logger)
#define ZM_DLOG_ZM_LL_ERROR(logger)    ZM_DLOG_ERROR(logger)
#define ZM_DLOG_ZM_LL_FATAL(logger)    ZM_DLOG_FATAL(logger)

// Note: Always logged.


#define ZM_LOG_IF(level, logger, condition)        !(condition) ? (void)0 : cpplog::helpers::VoidStreamClass() & ZM_LOG_##level(logger)
#define ZM_LOG_IF_NOT(level, logger, condition)    !!(condition) ? (void)0 : cpplog::helpers::VoidStreamClass() & ZM_LOG_##level(logger)

//#define ZM_DLOG_IF(level, logger, condition)        !(condition) ? (void)0 : cpplog::helpers::VoidStreamClass() & ZM_DLOG_##level(logger)
...
#else
#define DLOG_TRACE(logger)  LOG_NOTHING(ZM_LL_TRACE, logger)
#define DLOG_DEBUG(logger)  LOG_NOTHING(ZM_LL_DEBUG, logger)
#define DLOG_INFO(logger)   LOG_NOTHING(ZM_LL_INFO,  logger)
#define DLOG_WARN(logger)   LOG_NOTHING(ZM_LL_WARN,  logger)
#define DLOG_ERROR(logger)  LOG_NOTHING(ZM_LL_ERROR, logger)
#endif
// */
/*
// Note: Always logged.
...
//      LOG(ZM_LL_FATAL, logger)
#define LOG_ZM_LL_TRACE(logger)    LOG_TRACE(logger)
#define LOG_ZM_LL_DEBUG(logger)    LOG_DEBUG(logger)
#define LOG_ZM_LL_INFO(logger)     LOG_INFO(logger)
#define LOG_ZM_LL_WARN(logger)     LOG_WARN(logger)
#define LOG_ZM_LL_ERROR(logger)    LOG_ERROR(logger)
#define LOG_ZM_LL_FATAL(logger)    LOG_FATAL(logger)

#define DLOG_ZM_LL_TRACE(logger)   DLOG_TRACE(logger)
#define DLOG_ZM_LL_DEBUG(logger)   DLOG_DEBUG(logger)
#define DLOG_ZM_LL_INFO(logger)    DLOG_INFO(logger)
#define DLOG_ZM_LL_WARN(logger)    DLOG_WARN(logger)
#define DLOG_ZM_LL_ERROR(logger)   DLOG_ERROR(logger)
#define DLOG_ZM_LL_FATAL(logger)   DLOG_FATAL(logger)
...
/*
// Assertion helpers.
#define LOG_ASSERT(logger, condition)           LOG_IF_NOT(ZM_LL_FATAL, logger, (condition)) << "Assertion failed: " #condition
#define DLOG_ASSERT(logger, condition)          DLOG_IF_NOT(ZM_LL_FATAL, logger, (condition)) << "Assertion failed: " #condition
...
#define __ZM_CHECK(logger, condition, print)   ZM_LOG_IF(ZM_LL_FATAL, logger, !(condition)) << "Check failed: " print ": "

#define ZM_CHECK(logger, condition)            __ZM_CHECK(logger, condition, #condition)
#define ZM_CHECK_EQUAL(logger, ex1, ex2)       __ZM_CHECK(logger, (ex1) == (ex2), #ex1 " == " #ex2)
#define ZM_CHECK_LT(logger, ex1, ex2)          __ZM_CHECK(logger, (ex1) < (ex2), #ex1 " < " #ex2)
#define ZM_CHECK_GT(logger, ex1, ex2)          __ZM_CHECK(logger, (ex1) > (ex2), #ex1 " > " #ex2)
#define ZM_CHECK_LE(logger, ex1, ex2)          __ZM_CHECK(logger, (ex1) <= (ex2), #ex1 " <= " #ex2)
#define ZM_CHECK_GE(logger, ex1, ex2)          __ZM_CHECK(logger, (ex1) >= (ex2), #ex1 " >= " #ex2)

#define ZM_CHECK_NE(logger, ex1, ex2)          __ZM_CHECK(logger, (ex1) != (ex2), #ex1 " != " #ex2)
#define ZM_CHECK_NOT_EQUAL(logger, ex1, ex2)   __ZM_CHECK(logger, (ex1) != (ex2), #ex1 " != " #ex2)

// String helpers.
#define ZM_CHECK_STREQ(logger, s1, s2)         __ZM_CHECK(logger, strcmp((s1), (s2)) == 0, "") << s1 << " == " << s2
#define ZM_CHECK_STRNE(logger, s1, s2)         __ZM_CHECK(logger, strcmp((s1), (s2)) != 0, "") << s1 << " != " << s2

// NULL helpers.
#define ZM_CHECK_NULL(logger, exp)             __ZM_CHECK(logger, (exp) == NULL, #exp " == NULL")
#define ZM_CHECK_NOT_NULL(logger, exp)         __ZM_CHECK(logger, (exp) != NULL, #exp " != NULL")


// Debug versions of above.
#ifdef _DEBUG
#define ZM_DCHECK(logger, condition)           ZM_CHECK(logger, condition)
#define ZM_DCHECK_EQUAL(logger, ex1, ex2)      ZM_CHECK_EQUAL(logger, ex1, ex2)
#define ZM_DCHECK_LT(logger, ex1, ex2)         ZM_CHECK_LT(logger, ex1, ex2)
#define ZM_DCHECK_GT(logger, ex1, ex2)         ZM_CHECK_GT(logger, ex1, ex2)
#define ZM_DCHECK_LE(logger, ex1, ex2)         ZM_CHECK_LE(logger, ex1, ex2)
#define ZM_DCHECK_GE(logger, ex1, ex2)         ZM_CHECK_GE(logger, ex1, ex2)
#define ZM_DCHECK_NE(logger, ex1, ex2)         ZM_CHECK_NE(logger, ex1, ex2)
#define ZM_DCHECK_NOT_EQUAL(logger, ex1, ex2)  ZM_CHECK_NOT_EQUAL(logger, ex1, ex2)
#define ZM_DCHECK_STREQ(logger, s1, s2)        ZM_CHECK_STREQ(logger, s1, s2)
#define ZM_DCHECK_STRNE(logger, s1, s2)        ZM_CHECK_STRNE(logger, s1, s2)
#define ZM_DCHECK_NULL(logger, exp)            ZM_CHECK_NULL(logger, exp)
#define ZM_DCHECK_NOT_NULL(logger, exp)        ZM_CHECK_NOT_NULL(logger, exp)
#else
#define ZM_DCHECK(logger, condition)           while(false) ZM_CHECK(logger, condition)
#define ZM_DCHECK_EQUAL(logger, ex1, ex2)      while(false) ZM_CHECK_EQUAL(logger, ex1, ex2)
#define ZM_DCHECK_LT(logger, ex1, ex2)         while(false) ZM_CHECK_LT(logger, ex1, ex2)
#define ZM_DCHECK_GT(logger, ex1, ex2)         while(false) ZM_CHECK_GT(logger, ex1, ex2)
#define ZM_DCHECK_LE(logger, ex1, ex2)         while(false) ZM_CHECK_LE(logger, ex1, ex2)
#define ZM_DCHECK_GE(logger, ex1, ex2)         while(false) ZM_CHECK_GE(logger, ex1, ex2)
#define ZM_DCHECK_NE(logger, ex1, ex2)         while(false) ZM_CHECK_NE(logger, ex1, ex2)
#define ZM_DCHECK_NOT_EQUAL(logger, ex1, ex2)  while(false) ZM_CHECK_NOT_EQUAL(logger, ex1, ex2)
#define ZM_DCHECK_STREQ(logger, s1, s2)        while(false) ZM_CHECK_STREQ(logger, s1, s2)
#define ZM_DCHECK_STRNE(logger, s1, s2)        while(false) ZM_CHECK_STRNE(logger, s1, s2)
#define ZM_DCHECK_NULL(logger, exp)            while(false) ZM_CHECK_NULL(logger, exp)
#define ZM_DCHECK_NOT_NULL(logger, exp)        while(false) ZM_CHECK_NOT_NULL(logger, exp)
#endif
